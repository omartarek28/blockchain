...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"actionchannel\")"
  ...
  echo
  echo "	byfn.sh generate -c actionchannel"
  echo "	byfn.sh up -c actionchannel -s couchdb"
  echo "        byfn.sh up -c actionchannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c actionchannel"
  echo "        byfn.sh upgrade -c actionchannel"
  echo
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.abosalah.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.abosalah.com
    docker cp -a orderer.abosalah.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.abosalah.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.abosalah.com

    for PEER in peer0.vw.abosalah.com peer1.vw.abosalah.com peer0.bmw.abosalah.com peer1.bmw.abosalah.com peer0.mb.abosalah.com peer1.mb.abosalah.com; do
...
function networkDown() {
  # stop mb containers also in addition to vw and bmw, in case we were running sample to add mb
  # stop kafka and zookeeper containers in case we're running with kafka consensus-type
  docker-compose -f $COMPOSE_FILE -f $COMPOSE_FILE_COUCH -f $COMPOSE_FILE_KAFKA -f $COMPOSE_FILE_RAFT2 -f $COMPOSE_FILE_ORG3 -f $COMPOSE_FILE_CA down --volumes --remove-orphans
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./mb-artifacts/crypto-config/ channel-artifacts/mb.json
...
# and two Peer Orgs (``VW`` & ``BMW``) each managing and maintaining two peer nodes.
...
# (``peer0.vw.abosalah.com`` & ``peer0.bmw.abosalah.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for VWMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/VWMSPanchors.tx -channelID $CHANNEL_NAME -asOrg VWMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for VWMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for BMWMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/BMWMSPanchors.tx -channelID $CHANNEL_NAME -asOrg BMWMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for BMWMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for MBMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/MBMSPanchors.tx -channelID $CHANNEL_NAME -asOrg MBMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for MBMSP..."
    exit 1
  fi
  echo
}
...
CLI_DELAY=3
# channel name defaults to "actionchannel"
...
# mb docker compose file
COMPOSE_FILE_ORG3=docker-compose-mb.yaml
