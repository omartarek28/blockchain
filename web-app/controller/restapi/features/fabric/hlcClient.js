...
exports.getMyPatents = async function (req, res, next) {
    ...
    try {
        ...
        // Get the patents for the member including their state
        for (let patentId of member.patents) { 
            ...
            var _jsnPatents = JSON.parse(_jsn.patents);
            _jsn.patents = _jsnPatents;
...
exports.addOrder = async function (req, res, next) {
    let method = 'createPatent';
    console.log(method+' req.body.owner is: '+req.body.ownerId );    
    let orderNo = '00' + Math.floor(Math.random() * 10000);
    let order = {};
    order = svc.createPatentTemplate(order);
    if (svc.m_connection === null) {svc.createMessageSocket();}
